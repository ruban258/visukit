...
import { startGateway, type ConnStatus } from '../src/lib/server/opcua/client.ts';
import { startWsServer } from '../src/lib/server/opcua/wsServer.ts';
import { openHistorian } from '../src/lib/server/opcua/historian.ts';
import { loadConfig, tagsByGroup } from '../src/lib/server/opcua/tags.ts';
...
async function main() {
	...
	// Convenience for the common single-server case: OPCUA_ENDPOINT/USER/PASS override it, so you
	// can point at a real PLC without editing config. Ignored when multiple servers are defined.
	...
	for (const s of servers) console.log(`  server "${s.id}" → ${s.endpoint}`);
